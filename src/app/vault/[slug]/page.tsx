...
export default function OfferPage() {
  ...
  const handleRequest = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Attempt to save to DB
      // We use a timeout or try/catch so a DB failure doesn't block the user
      await captureVaultLead(email, offer.slug);
      
      // 2. Set a session flag for the Thank You page security check
      if (typeof window !== "undefined") {
        sessionStorage.setItem("is_human", "true");
      }
    } catch (err) {
      console.error("Database sync failed, proceeding to redirect anyway.");
    } finally {
      // 3. ALWAYS redirect, even if the DB hit failed
      // This ensures you don't lose the CPA commission
      router.push(`/thank-you?offer=${offer.slug}`);
    }
  };
              ...
              <CardContent className="p-8">
                <form onSubmit={handleRequest} className="space-y-4">
                  ...
                  <Button 
                    type="submit"
                    disabled={isLoading}
                    className="w-full h-16 text-lg font-black uppercase italic bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl transition-all active:scale-95"
                  >
                    {isLoading ? (
